...
const MapComponent = ({ mapboxToken, startCoords, endCoords }) => {
  ...
  useEffect(() => {
    if (typeof window === 'undefined' || userLocation || mapInitializedRef.current) return;
    ...
    const getUserLocation = async () => {
      ...
      try {
        ...
        setUserLocation([40.7128, -74.0060]); // NYC default
      } finally {
  ...
  useEffect(() => {
    if (typeof window === 'undefined' || !mapContainerRef.current || !mapboxToken || 
        !userLocation || mapInitializedRef.current) {
      return;
    }
    ...
    try {
      ...
      const map = L.map(container).setView(userLocation, 15);
  ...
  useEffect(() => {
    ...
    console.log("Setting up routing with endpoints:", { start: userLocation, end: endCoords });
    setupRouting();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const setupRouting = () => {
    ...
    try {
      ...
      if (!L.Routing || !L.Routing.mapbox) {
        ...
        setError("Routing libraries not properly loaded. Please refresh the page.");
        return;
      }
        ...
        try {
          ...
          control.on('routingerror', function(e) {
            ...
            try {
              ...
              control.getRouter = function() { return osrmRouter; };
              control.route();
            } catch (fallbackError) {
              console.error('Fallback routing error:', fallbackError);
              setError('Unable to calculate a route. Please try different locations.');
            }
          });
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          if (mapInstanceRef.current) {
            ...
            if (mapInstanceRef.current._userMarker) {
              mapInstanceRef.current._userMarker.setLatLng([latitude, longitude]);
            }
          }
        },
        (error) => {
