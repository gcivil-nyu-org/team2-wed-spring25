...
import { useUserLocationWithNYCCheck, isWithinNYC, NYC_BOUNDS } from "@/hooks/location"
...
export function RouteProvider({ children }) {
    ...
    }, [useCurrentLocation, userLocation, canUseCurrentLocation, routeCalculated]);

    // Centralized validation function
    const validateForm = (departure, departureCoordinates, destination, destinationCoordinates, formUseCurrentLocation) => {
        if (formUseCurrentLocation) {
            if (!canUseCurrentLocation || !userLocation) {
                return {
                    isValid: false,
                    error: "Cannot use current location. Your location is unavailable or outside NYC boundaries."
                };
            }
