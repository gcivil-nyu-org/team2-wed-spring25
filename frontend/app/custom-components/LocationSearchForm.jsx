    ...
    try {
      const response = await fetch(
        `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(
          departure
        )}.json?access_token=${mapboxToken}&types=address,place,poi&limit=5`
      );
      ...
      showError(
        ...
        error.message ||
          "Failed to fetch location suggestions. Please check your internet connection.",
...
const getCurrentLocation = async () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (
      ...
      !isWithinNYC(departureCoordinates)
    ) {
