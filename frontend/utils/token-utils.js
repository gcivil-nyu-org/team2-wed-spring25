...
export function getTokenTimeRemaining(token) {
  ...
  try {
    ...
    return decoded.exp > currentTime ? Math.floor(decoded.exp - currentTime) : 0;
  } catch (error) {
    console.error('Error decoding token:', error);
    return 0;
  }
 ...
 */
export function setupTokenRefresh(refreshCallback, token, refreshThresholdSeconds = 300) {
...
export async function refreshDjangoToken() {
  try {
    ...
    const response = await apiPost('/api/token/refresh/', { refresh: refreshToken });
