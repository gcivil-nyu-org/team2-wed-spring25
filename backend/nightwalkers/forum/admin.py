...
class PostAdmin(admin.ModelAdmin):
    list_display = ('title', 'user', 'content', 'image_urls', 
                    'date_created', 'date_updated')  # Fields to display in the list view
    list_filter = ('date_created', 'user')  # Filters to add in the sidebar
    search_fields = ('title', 'content')  # Fields to search by
    readonly_fields = ('date_created', 'date_updated')  # Fields that should be read-only

    # Optionally, you can customize the form fields
    fieldsets = (
        (None, {
            'fields': ('user', 'title', 'content', 'image_urls')
        }),
        ('Dates', {
