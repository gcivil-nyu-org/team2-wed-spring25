...
class ReportPostInline(admin.TabularInline):
    ...
    def reporting_user_link(self, obj):
        ...
        return format_html('<a href="{}">{}</a>', url, obj.reporting_user.get_full_name())
...
class PostAdmin(admin.ModelAdmin):
    ...
    def post_type_badge(self, obj):
        if obj.is_repost:
            return format_html(
                '<span style="background-color: #f39c12; color: #fff; padding: 3px 8px; '
    ...
    def image_count(self, obj):
        if obj.image_urls:
            return format_html('<span style="color: #27ae60;">{} image(s)</span>', len(obj.image_urls))
    ...
    def image_display(self, obj):
        ...
        for url in obj.image_urls:
            html += f'<div style="margin-bottom: 10px;"><img src="{url}" style="max-width: 200px; max-height: 200px; border-radius: 5px;" /><br/><a href="{url}" target="_blank">View full size</a></div>'
        html += '</div>'
    ...
    def original_post_link(self, obj):
        if obj.is_repost and obj.original_post:
            ...
            return format_html(
                '<a href="{}" style="background-color: #f39c12; color: #fff; padding: 5px 10px; border-radius: 5px; text-decoration: none; font-size: 12px;">View Original Post</a>',
                url)
        return "-"
    ...
    def reposted_by_link(self, obj):
        if obj.reposted_by:
            ...
            return format_html('<a href="{}">{}</a>', url, obj.reposted_by.get_full_name())
        return "-"
...
class CommentAdmin(admin.ModelAdmin):
    ...
    search_fields = ('content', 'user__email', 'user__first_name', 'user__last_name', 'post__content')
    readonly_fields = (
        'date_created',
        'date_updated',
        'user_link',
    ...
    def comment_type_badge(self, obj):
        if obj.parent_comment:
            return format_html(
                '<span style="background-color: #9b59b6; color: #fff; padding: 3px 8px; '
    ...
    def post_link(self, obj):
        ...
        return format_html('<a href="{}">{}</a>', url,
                           obj.post.content[:50] + "..." if len(obj.post.content) > 50 else obj.post.content)
    ...
    def parent_comment_link(self, obj):
        if obj.parent_comment:
            ...
            return format_html('<a href="{}">{}</a>', url, obj.parent_comment.content[:50] + "..." if len(
                obj.parent_comment.content) > 50 else obj.parent_comment.content)
        return "-"
    ...
    fieldsets = (
        ("Comment Content", {
            'fields': ('content', 'likes_count', 'replies_count')
        }),
        ("Comment Information", {
            'fields': ('user_link', 'date_created', 'date_updated')
        }),
...
class LikeAdmin(admin.ModelAdmin):
    ...
    list_filter = ('date_created', 'like_type', ('user', admin.RelatedOnlyFieldListFilter))
    search_fields = ('user__email', 'user__first_name', 'user__last_name', 'post__content')
    ...
    def post_preview(self, obj):
        ...
        if obj.post.is_repost:
            badge_html = '<span style="background-color: #f39c12; color: #fff; padding: 2px 5px; border-radius: 3px; margin-right: 5px; font-size: 12px;">Repost</span>'
        else:
            badge_html = '<span style="background-color: #3498db; color: #fff; padding: 2px 5px; border-radius: 3px; margin-right: 5px; font-size: 12px;">Original</span>'
    ...
    def post_link(self, obj):
        ...
        return format_html('<a href="{}">{}</a>', url,
                           obj.post.content[:100] + "..." if len(obj.post.content) > 100 else obj.post.content)
...
class CommentLikeAdmin(admin.ModelAdmin):
    ...
    list_filter = ('date_created', 'like_type', ('user', admin.RelatedOnlyFieldListFilter))
    search_fields = ('user__email', 'user__first_name', 'user__last_name', 'comment__content', 'comment__post__content')
    ...
    def comment_preview(self, obj):
        ...
        if obj.comment.parent_comment:
            badge_html = '<span style="background-color: #9b59b6; color: #fff; padding: 2px 5px; border-radius: 3px; margin-right: 5px; font-size: 12px;">Reply</span>'
        else:
            badge_html = '<span style="background-color: #2ecc71; color: #fff; padding: 2px 5px; border-radius: 3px; margin-right: 5px; font-size: 12px;">Comment</span>'
    ...
    def comment_link(self, obj):
        url = reverse('admin:forum_comment_change', args=[obj.comment.id])
        return format_html('<a href="{}">{}</a>', url,
                           obj.comment.content[:100] + "..." if len(obj.comment.content) > 100 else obj.comment.content)

    comment_link.short_description = 'Comment'

    def post_link(self, obj):
        url = reverse('admin:forum_post_change', args=[obj.comment.post.id])
        return format_html('<a href="{}">{}</a>', url, obj.comment.post.content[:100] + "..." if len(
            obj.comment.post.content) > 100 else obj.comment.post.content)
...
class ReportPostAdmin(admin.ModelAdmin):
    ...
    list_filter = (
    'is_repost', ('reporting_user', admin.RelatedOnlyFieldListFilter), ('post_owner', admin.RelatedOnlyFieldListFilter))
    search_fields = ('post__content', 'reporting_user__email', 'post_owner__email')
    readonly_fields = ('post_link', 'reporting_user_link', 'post_owner_link', 'is_repost')
    ...
    fieldsets = (
        ("Report Information", {
            'fields': ('post_link', 'reporting_user_link', 'post_owner_link', 'is_repost')
        }),
        ("Technical Details", {
...
class ReportCommentAdmin(admin.ModelAdmin):
    ...
    list_filter = ('date_reported', ('reporting_user', admin.RelatedOnlyFieldListFilter))
    search_fields = ('comment__content', 'reporting_user__email', 'reason')
    readonly_fields = ('date_reported', 'comment_link', 'reporting_user_link', 'post_link')
    ...
    fieldsets = (
        ("Report Information", {
            'fields': ('comment_link', 'reporting_user_link', 'reason', 'date_reported')
        }),
        ("Related Post", {
            'fields': ('post_link',),
        }),
