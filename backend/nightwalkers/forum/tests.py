...
class CreatePostTests(TestCase):
    def setUp(self):
        ...
        self.existing_post = Post.objects.create(
            user=self.user, title="Test Post", content="Original content", image_urls=[]
        )
    ...
    def test_missing_required_fields(self):
        ...
        for field, partial_data in test_cases:
            ...
            self.assertEqual(
                response.status_code, 400, f"Expected 400 when missing {field}"
            )
...
class GetUserDataTests(TestCase):
    def setUp(self):
        ...
        Follow.objects.create(main_user=self.other_user, following_user=self.user)
    ...
    def test_successful_get_user_data(self):
        response = self.client.get(reverse("get_user_data"), {"user_id": self.user.id})
        data = self.parse_response(response)
    ...
    def test_invalid_user_id(self):
        response = self.client.get(reverse("get_user_data"), {"user_id": 9999})
        data = self.parse_response(response)
    ...
    def test_zero_counts(self):
        ...
        response = self.client.get(reverse("get_user_data"), {"user_id": new_user.id})
        data = self.parse_response(response)
...
class GetPostsTests(TestCase):
    def setUp(self):
        ...
        Comment.objects.create(
            user=self.other_user, post=self.post2, content="Comment 2"
        )
    ...
    def test_get_posts_with_pagination(self):
        ...
        for i in range(3, 8):
            Post.objects.create(
                user=self.user, title=f"Post {i}", content=f"Content {i}"
            )

        response = self.client.get(
            reverse("get_posts"), {"user_id": self.user.id, "offset": 0, "limit": 3}
        )
        data = self.parse_response(response)
    ...
    def test_get_posts_posts_setting(self):
        response = self.client.get(
            reverse("get_posts"), {"user_id": self.user.id, "settings_type": "posts"}
        )
        data = self.parse_response(response)
    ...
    def test_get_posts_reactions_setting(self):
        response = self.client.get(
            reverse("get_posts"),
            {"user_id": self.user.id, "settings_type": "reactions"},
        )
        data = self.parse_response(response)
    ...
    def test_get_posts_reports_setting(self):
        response = self.client.get(
            reverse("get_posts"), {"user_id": self.user.id, "settings_type": "reports"}
        )
        data = self.parse_response(response)
    ...
    def test_get_posts_comments_setting(self):
        response = self.client.get(
            reverse("get_posts"), {"user_id": self.user.id, "settings_type": "comments"}
        )
        data = self.parse_response(response)
    ...
    def test_get_posts_like_info(self):
        ...
        post1_data = next(
            (p for p in data["posts"] if p["user_id"] == self.user.id), None
        )
        post2_data = next(
            (p for p in data["posts"] if p["user_id"] == self.other_user.id), None
        )
    ...
    def test_invalid_user_id(self):
        response = self.client.get(reverse("get_posts"), {"user_id": 9999})
        self.assertEqual(response.status_code, 404)
...
class GetPostTests(TestCase):
    ...
    def test_get_post_with_no_comments(self):
        new_post = Post.objects.create(
            user=self.user, title="No Comments", content="Test"
        )
    ...
    def test_get_post_with_no_images(self):
        new_post = Post.objects.create(
            user=self.user, title="No Images", content="Test", image_urls=[]
        )
        url = reverse("get_post", kwargs={"post_id": new_post.id})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
...
class CommentViewTests(TestCase):
    def setUp(self):
        ...
        self.post = Post.objects.create(
            user=self.user1, title="Test Post", content="Test Content"
        )

        # Create test comments
        self.parent_comment = Comment.objects.create(
            user=self.user1, post=self.post, content="Parent comment"
        )
    ...
    def test_create_comment_success(self):
        data = {"user_id": self.user1.id, "content": "New comment"}
        response = self.client.post(
    ...
    def test_get_comments_paginated(self):
        ...
        for i in range(10):
            Comment.objects.create(
                user=self.user1, post=self.post, content=f"Comment {i}"
            )
    ...
    def test_get_replies_to_comment(self):
        response = self.client.get(
            reverse("comments", args=[self.post.id]),
            {"user_id": self.user1.id, "parent_comment_id": self.parent_comment.id},
        )
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
        self.assertEqual(len(data["comments"]), 1)
    ...
    def test_comment_like_info_included(self):
        response = self.client.get(
            reverse("comments", args=[self.post.id]), {"user_id": self.user1.id}
        )
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
        ...
        parent_data = next(
            c for c in data["comments"] if c["id"] == self.parent_comment.id
        )
...
class LikePostTests(TestCase):
    ...
    def test_like_post_success(self):
        data = {"user_id": self.user2.id, "is_liked": True, "like_type": "Like"}
        response = self.client.post(
    ...
    def test_unlike_post(self):
        ...
        data = {"user_id": self.user2.id, "is_liked": False, "like_type": "Like"}
        response = self.client.post(
            reverse("like_post", args=[self.post.id]),
            data=json.dumps(data),
            content_type="application/json",
    ...
    def test_change_like_type(self):
        ...
        data = {"user_id": self.user2.id, "is_liked": True, "like_type": "Heart"}
        response = self.client.post(
            reverse("like_post", args=[self.post.id]),
            data=json.dumps(data),
            content_type="application/json",
    ...
    def test_multiple_like_types(self):
        ...
        for like_type in like_types:
            data = {"user_id": self.user2.id, "is_liked": True, "like_type": like_type}
            response = self.client.post(
                reverse("like_post", args=[self.post.id]),
                data=json.dumps(data),
                content_type="application/json",
...
class FollowUnfollowTests(TestCase):
    ...
    def test_invalid_user(self):
        data = {"user_id": 9999, "follow": True}  # Non-existent user
        response = self.client.post(
            reverse("follow_unfollow_user", args=[self.user2.id]),
            data=json.dumps(data),
            content_type="application/json",
...
class LikeCommentTests(TestCase):
    def setUp(self):
        ...
        self.comment = Comment.objects.create(
            user=self.user1, post=self.post, content="Test Comment"
        )
    ...
    def test_like_comment_success(self):
        ...
        # Verify like was created
        self.assertTrue(
            CommentLike.objects.filter(user=self.user2, comment=self.comment).exists()
        )
    ...
    def test_unlike_comment(self):
        ...
        data = {"user_id": self.user2.id, "is_liked": False}
        response = self.client.post(
            reverse("like_comment", args=[self.comment.id]),
            data=json.dumps(data),
            content_type="application/json",
    ...
    def test_update_like_type(self):
        ...
        CommentLike.objects.create(
            user=self.user2, comment=self.comment, like_type="like"
        )

        data = {"user_id": self.user2.id, "is_liked": True, "like_type": "love"}
        response = self.client.post(
            reverse("like_comment", args=[self.comment.id]),
            data=json.dumps(data),
            content_type="application/json",
    ...
    def test_invalid_user(self):
        data = {"user_id": 9999, "is_liked": True}  # Non-existent user
        response = self.client.post(
            reverse("like_comment", args=[self.comment.id]),
            data=json.dumps(data),
            content_type="application/json",
    ...
    def test_invalid_comment(self):
        data = {"user_id": self.user2.id, "is_liked": True}
        response = self.client.post(
...
class ReportPostTests(TestCase):
    def setUp(self):
        ...
        self.original_post = Post.objects.create(
            user=self.post_owner, title="Original Post", content="Original content"
        )
    ...
    def test_invalid_users(self):
        ...
        data = {"reporting_user_id": 9999, "post_owner_id": self.post_owner.id}
        response = self.client.post(
            reverse("report_post", args=[self.original_post.id]),
            data=json.dumps(data),
            content_type="application/json",
        ...
        data = {"reporting_user_id": self.reporting_user.id, "post_owner_id": 9999}
        response = self.client.post(
            reverse("report_post", args=[self.original_post.id]),
            data=json.dumps(data),
            content_type="application/json",
...
class ReportCommentTests(TestCase):
    def setUp(self):
        ...
        # Create a post and comment for testing
        self.post = Post.objects.create(
            user=self.comment_author, title="Test Post", content="Test Content"
        )
        self.comment = Comment.objects.create(
            user=self.comment_author, post=self.post, content="Test Comment"
        )
    ...
    def test_report_comment_success(self):
        ...
        self.assertEqual(
            ...
            "Comment reported successfully. Karma decreased by 5.",
        )

        # Verify report was created
        self.assertTrue(
            ReportComment.objects.filter(
                comment=self.comment, reporting_user=self.reporting_user
            ).exists()
        )
    ...
    def test_report_comment_without_reason(self):
        data = {"comment_id": self.comment.id, "user_id": self.reporting_user.id}
        response = self.client.post(
            reverse("report_comment"),
            data=json.dumps(data),
            content_type="application/json",
    ...
    def test_missing_required_fields(self):
        ...
        data = {"user_id": self.reporting_user.id}
        response = self.client.post(
            reverse("report_comment"),
            data=json.dumps(data),
            content_type="application/json",
        ...
        response_data = json.loads(response.content)
        self.assertEqual(
            response_data["error"], "Both comment_id and user_id are required."
        )
    ...
    def test_invalid_user(self):
        data = {"comment_id": self.comment.id, "user_id": 9999}  # Non-existent user
        response = self.client.post(
            reverse("report_comment"),
            data=json.dumps(data),
            content_type="application/json",
...
class DeleteCommentViewTests(TestCase):
    ...
    def test_delete_comment_without_replies(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            Comment.objects.filter(post=self.post).count(), comments_before - 1
        )
    ...
    def test_delete_comment_wrong_method_get(self):
        ...
        self.assertEqual(response.status_code, 405)
        self.assertEqual(
            json.loads(response.content)["error"], "Only DELETE requests are allowed."
        )
...
class DeletePostViewTests(TestCase):
    ...
    def test_delete_post_success(self):
        ...
        response_data = json.loads(response.content)
        self.assertEqual(
            response_data["message"], "Post and its reposts deleted successfully."
        )
