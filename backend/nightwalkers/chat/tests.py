...
class ChatViewsTestCase(TestCase):
    def setUp(self):
        ...
        Message.objects.create(
            chat=self.chat, sender=self.user2, content="Hi there", read=False
        )

    def test_get_mutual_follows_with_chats(self):
        url = reverse(
            "get_mutual_follows_with_chats", kwargs={"user_id": self.user1.id}
        )
        ...
        self.assertEqual(
            data["data"][0]["unread_count"], 1
        )  # One unread message from user2

    def test_read_user_messages(self):
        url = reverse(
            "read_user_messages",
            kwargs={"chat_uuid": self.chat.uuid, "sender_id": self.user2.id},
        )
        ...
        unread_count = Message.objects.filter(
            chat=self.chat, sender=self.user2, read=False
        ).count()
    ...
    def test_read_user_messages_invalid_chat(self):
        ...
        url = reverse(
            "read_user_messages",
            kwargs={"chat_uuid": invalid_uuid, "sender_id": self.user2.id},
        )
        response = self.client.post(url)
    ...
    def test_get_mutual_follows_user_not_found(self):
        ...
        url = reverse(
            "get_mutual_follows_with_chats", kwargs={"user_id": invalid_user_id}
        )
        response = self.client.get(url)
