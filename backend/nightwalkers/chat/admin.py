...
class ChatAdmin(admin.ModelAdmin):
    ...
    readonly_fields = (
    'created_at', 'uuid', 'user1_link', 'user2_link', 'message_count', 'latest_message_preview', 'message_list')
    ...
    def view_messages(self, request, chat_id):
        ...
        context = {
            **self.admin_site.each_context(request),
            ...
            'chat': chat,
            'title': f'Messages in chat between {chat.user1.get_full_name()} and {chat.user2.get_full_name()}',
        }
    ...
    def get_messages_json(self, request, chat_id):
        ...
        try:
            ...
            messages = Message.objects.filter(chat=chat).order_by('-timestamp')[offset:offset + page_size]
    ...
    def latest_message(self, obj):
        ...
        if latest:
            return format_html(
                ...
                latest.content,
                latest.content[:40] + "..." if len(latest.content) > 40 else latest.content,
    ...
    def latest_message_preview(self, obj):
        ...
        if latest:
            ...
            return format_html(
                '<div style="margin-bottom: 10px;"><strong><a href="{}">{}</a></strong> - {}</div>'
                '<div style="padding: 10px; background-color: #f5f5f5; border-radius: 5px;">{}</div>',
                sender_url, sender_name, latest.timestamp.strftime("%b %d, %Y, %H:%M"),
                latest.content
            )
        return "No messages"
    ...
    def message_list(self, obj):
        ...
        return format_html(
            ...
            'padding: 8px 15px; text-decoration: none; border-radius: 4px; display: inline-block; '
            'margin-top: 10px;">View All Messages</a>',
            url
        )
    ...
    fieldsets = (
        ...
        ('Chat Statistics', {
            'fields': ('message_count', 'latest_message_preview')
        }),
        ('Messages', {
            'fields': ('message_list',)
        }),
...
class MessageAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('timestamp', 'sender_link', 'chat_link', 'user_names', 'other_messages')
    ...
    def user_names(self, obj):
        ...
        return format_html(
            ...
            user1_url, obj.chat.user1.get_full_name(),
            user2_url, obj.chat.user2.get_full_name()
        )
    ...
    def other_messages(self, obj):
        ...
        earlier_messages = list(queryset.filter(
            timestamp__lt=current_msg_time
        ).order_by('-timestamp')[:5])

        later_messages = list(queryset.filter(
            timestamp__gt=current_msg_time
        ).order_by('timestamp')[:5])
        ...
        for msg in sorted(context_messages, key=lambda x: x.timestamp):
            ...
            html += f'''
            <tr style="{row_style}">
                <td style="padding: 8px; width: 15%;"><a href="{sender_url}">{msg.sender.get_full_name()}</a></td>
                <td style="padding: 8px; width: 65%;"><a href="{msg_url}">{msg.content}</a></td>
                <td style="padding: 8px; width: 20%;"><small>{msg.timestamp.strftime("%b %d, %Y, %H:%M")}</small></td>
            </tr>
            '''
    ...
    fieldsets = (
        ...
        ('Chat Information', {
            'fields': ('chat_link', 'user_names')
        }),
        ('Message Context', {
            'fields': ('other_messages',)
        }),
    )
